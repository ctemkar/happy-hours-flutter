...
class _HappyHoursScreenState extends State<HappyHoursScreen> {
  ...
  /*void _openBusinessDetails(HappyHourPlace business) async {
    ...
    if (kIsWeb) {
      // On Web, open the asset URL in a new tab
      final uri = Uri.base.resolve('assets/output_html/$filename');
      if (!await launchUrl(uri, webOnlyWindowName: '_blank')) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not open page in a new tab')),
        );
      }
      return;
    }

    // On mobile: push a page that loads the HTML asset and shows it in a webview
