...
class _HappyHoursScreenState extends State<HappyHoursScreen> {
  ...
  void _openBusinessDetails(HappyHourPlace business) async {
    ...
    // 🚀 Use the absolute path to your XAMPP server
    final String businessUrl = "http://localhost/happy_hours_application/output_html/$filename";
    ...
    if (kIsWeb) {
      ...
      if (await canLaunchUrl(Uri.parse(businessUrl))) {
        ...
        debugPrint("Could not launch $businessUrl");
      }
      return;
    }
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => BusinessHtmlPage(filename: filename, title: business.name),
      ),
    );
  }
...
class _BusinessHtmlPageState extends State<BusinessHtmlPage> {
  ...
  @override
Widget build(BuildContext context) {
  // ✅ Fixed: Point to XAMPP local server instead of local file path
  final iframeUrl = "http://localhost/happy_hours_application/output_html/${widget.filename}";
  ...
  return Scaffold(
    ...
    body: Stack(
      children: [
        InAppWebView(
          initialUrlRequest: URLRequest(url: WebUri(iframeUrl)), // ✅ WebUri works with newer flutter_inappwebview
        ...
        // ✅ Show error overlay if something goes wrong
        if (error != null)
          Center(
            child: Container(
              margin: const EdgeInsets.all(20),
