    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => BusinessEditPage(
          ...
          onSaveSuccess: () {
            // Reload the HTML after successful save
            setState(() {
              _htmlFuture = _loadHtml(_filePathFromExactName(widget.businessName));
            });
          },
...
class _BusinessEditPageState extends State<BusinessEditPage> {
  ...
  Future<void> _fetchBusinessData() async {
    ...
    try {
      final resp = await http.post(
        Uri.parse("https://app.lovehappyhours.com/happy-hours-api/get_business_data"),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({
          "business_name": widget.businessName,
          if (widget.businessEmail != null) "email": widget.businessEmail,
      ...
      debugPrint('Fetch data response: ${resp.statusCode} ${resp.body}');

      if (resp.statusCode == 200) {
        final body = jsonDecode(resp.body);
        if (body["success"] == true && body["data"] != null) {
          setState(() {
            _businessData = body["data"];
            // Create controllers for each field
            _businessData.forEach((key, value) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Failed to fetch business data (${resp.statusCode})."),
  ...
  bool _isLongTextField(String key) {
    final lowerKey = key.toLowerCase();
    return lowerKey.contains('description') ||
        lowerKey.contains('address') ||
        lowerKey.contains('details') ||
        lowerKey.contains('about') ||
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _isSaving ? null : _saveBusinessData,
          ),
        ],
      ),
      body: _isLoading
              ...
              child: ListView(
                ...
                children: [
                  ...
                  ..._controllers.entries.map((entry) {
                    ...
                    final isLongField = _isLongTextField(fieldName);
