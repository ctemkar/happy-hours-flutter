...
IMAGES_BASE_URL = "http://localhost/happy_hours_application/business_images/"  # keep trailing slash
...
def fix_image_link(image_link: str) -> str:
    """
    Normalize image URLs for localhost.

    Cases handled:
    - old customercallsapp.com links
    - old app.lovehappyhours.com links
    - raw filenames
    - already-correct localhost links
    - external CDN/image URLs stay untouched
    """
    if not image_link:
        return ""
    ...
    try:
        # Already correct localhost URL
        if image_link.startswith("http://localhost/happy_hours_application/business_images/"):
            return image_link

        # Old production/custom domain URLs -> map to localhost
        if (
            image_link.startswith("https://customercallsapp.com")
            or image_link.startswith("http://customercallsapp.com")
            or image_link.startswith("https://app.lovehappyhours.com/business_images/")
            or image_link.startswith("http://app.lovehappyhours.com/business_images/")
        ):
            filename = os.path.basename(urlparse(image_link).path)
            filename = quote(filename)
            return IMAGES_BASE_URL + filename

        # If DB only contains filename like abc.jpg
        if "/" not in image_link and "\\" not in image_link:
            return IMAGES_BASE_URL + quote(image_link)

    except Exception:
        return image_link

    # Leave external full URLs untouched
    return image_link
